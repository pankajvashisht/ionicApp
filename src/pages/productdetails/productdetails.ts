...
import { IonicPage, NavController,ModalController ,Content,Slides,NavParams,AlertController,ToastController } from 'ionic-angular';
...
export class ProductdetailsPage {
  ...
  constructor(private socialSharing: SocialSharing,private apis :ApisProvider,private modalCtrl:ModalController, private session:SessionProvider,public navCtrl: NavController, public navParams: NavParams,public alterctrl:AlertController,private toast:ToastController) {
  ...
  send_query(){
    ...
    let modal = this.modalCtrl.create('SendqueryPage',{vender_id:this.product_details.user_id,type:type,table_id:this.product_details.id});
    modal.present();
  }
  ...
  share(){
    this.socialSharing.share(this.product_details.name,this.product_details.detail,this.product_details.media[0].file,'http://www.agrohypermarket.net/product/'+this.product_details.id).then(() => {
